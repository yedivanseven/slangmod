...
what it :doc:`can do <clean>`, we will thus specify the format :mod:`slangmod`
expects the text data to be in and where it expects it to be.
...
not, the name of that row is "**text**" but, as we will see later, this can be
configured.
...
.. note::
   ...
   you can still use the :doc:`clean` command to simply copy files from some
   source directory into the **corpus** folder.
